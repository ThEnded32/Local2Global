...
from my_codes.local2globalUNet import Local2GlobalUNet3Dv3
...
class Local2Global3DV3HR(pl.LightningModule):
    def __init__(
        self,
        epochs,
        in_channels=2,
        ...
        ### Model ### b, c, h, w, d, stem = 10, 1, 128, 128, 96, 24
        self.model = Local2GlobalUNet3Dv3()
    ...
    def forward(self, voided_image: Tensor, mask: Tensor) -> Tensor:
        if self.in_channels == 2:
            ...
            x = voided_image
        x_padded = torch.nn.functional.pad(x, (0,0,8,8,8,8), mode='reflect') # to be divisible by the window size or the number of groups
        out = self.model(x_padded)
        out = out[:, :, 8:-8,  8:-8, :] 
        return out* mask + voided_image * (~mask)
...
if __name__ == "__main__":
    ...
    # Training Epochs
    parser.add_argument("-e", "--epochs", dest="epochs", type=int, help="Epochs for trainings", default=20)
    # Batch Size
    parser.add_argument("-b", "--batch-size", dest="batch_size", type=int, help="Batch size per iteration", default=2)
    # Dataset
    parser.add_argument(
        "-d",
        "--dataset",
    ...
    # Train-Validation split percentage
    parser.add_argument("-cs", "--crop-shape", dest="crop_shape", type=int, nargs="+", default=[208, 208, 144], help="Crop shape")
    # Checkpoint path
    parser.add_argument("-cp", "--ckpt-path", dest="ckpt_path", type=str, default="local2global_submitted_checkpoint.ckpt", help="Checkpoint path")

    #### Set Parameters / Initialize ###
    args = parser.parse_args()  # Get commandline arguments
    print(args)
    name = "Local2GlobalV3HR"
    epochs = args.epochs
    ...
    ### Model and Training ###
    lighting_module = Local2Global3DV3HR(epochs)
